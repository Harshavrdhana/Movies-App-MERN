  ...
  try {
    ...
    if (movie) {
      const alreadyReviewed = movie.reviews.find(
        (r) => r.user.toString() === req.user._id.toString() // checks already reviewed or not
      )
      ...
      movie.numReviews = movie.reviews.length // update the no. of reviews
      movie.rating = movie.reviews.reduce((acc, item) => item.rating + acc, 0) / movie.reviews.length; //calculate the average of rating
...
const deleteComment = async (req, res) => {
  try {
    ...
    if (reviewIndex === -1) {
      return res.return(404).json({ message: "Comment not found" })
    }
